...
let FORM_INPUT_API = {
    URL_GET_FORM_INPUT: '/form_input/get_form_input',
    //URL_MGET_FORM_INPUT: '/form_input/mget_form_input',
    URL_GET_FORM_INPUT_LIST: '/form_input/get_form_input_list',
};
    ...
    getSelectOptionList(thisSel, dataPropsName) {
        ...
        $.ajax({
            ...
            success: function(remoteData, status) {
                ...
                basic.getField(
                    remoteData,
                    basic.getField(detailContent, ['data_path'], [])
                ).forEach((option) => {
                    optionList.push({
                        display: basic.getField(
                            option,
                            basic.getField(detailContent, ['display_path'], 'display')
                        ),
                        value: basic.getField(
                            option,
                            basic.getField(detailContent, ['value_path'], 'value')
                        ),
                    });
                });
